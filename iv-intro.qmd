...
$$
- Assumption: $\hat{\beta}^{OLS}$ is consistent for $\beta$ if:
$$
\mathbb{E}[u_i|X_i] = 0
$$
i.e. $X_i$ is **exogenous** and contains no information on $u_i$.
- When $\mathbb{E}[u_i|X_i] \ne 0$, $X_i$ is **endogenous** and all $\hat{\beta}^{OLS}$ is [inconsistent]{.fg} (invalid even in large samples).
...
$$
Y_{i} = \beta_{0} + \beta_{1} X_{i} + \beta_{2} A_{i} + u_{i}
$$
where $A_{i}$ is the *omitted* (but relevant) variable and $\mathbb{E}[u_i|X_i A_i] =0$
- If we omit $A_{i}$ we have:
$$
Y_{i} = \alpha_0 + \alpha_1 X_i + v_{i}
$$
where $v_{i}$ is the composite error $v_{i} = \beta_{2} A_{i} + u_{i}$
- We can show that (exercise):
$$
\text{plim } \hat{\alpha}_{1}^{OLS} = \beta_{1} + \frac{\text{Cov}(x_i, A_{i})}{\mathbb{V}(x)}
$$
...
- We can estimate $\beta_1$ using a two-step procedure:
    - [First stage regression]{.fg}: Regress $X$ on $Z$ (reduced form equation for $X$):
      $$
      X_{i} = \gamma_0 + \gamma_1 Z_{i} + v_{i}
      $$
    ...
    - [Second stage regression]{.fg}: Regress $Y_{i}$ on $\hat{X}_{i}$
      ...
      [Note:]{.note} OLS s.e. of second stage regression is incorrect. Use statistical software
...
## IV: Instrument Validity {.small-slide}

Exclusion restriction may be an issue.
